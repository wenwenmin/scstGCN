...
def cv2_detect_contour_new(img,
	CANNY_THRESH_1 = 100,
	CANNY_THRESH_2 = 200,
	apertureSize=5,
	L2gradient = True,
	all_cnt_info=False,
	full_image_contour=True):  # 添加新参数 full_image_contour
	if len(img.shape) == 3:
		gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	elif len(img.shape) == 2:
		gray = (img * ((1, 255)[np.max(img) <= 1])).astype(np.uint8)
	...
	if full_image_contour:
		# 创建一个包围整张图片的矩形轮廓
		h, w = img.shape[:2]
		contour = np.array([[[0, 0]], [[0, h-1]], [[w-1, h-1]], [[w-1, 0]]], dtype=np.int32)
		return contour
