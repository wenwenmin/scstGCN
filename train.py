...
def get_model(
    ...
    if (epochs is not None) and (epochs > 0):
        model, hist, trainer = train_model(
            model=model,
            model_class=model_class, model_kwargs=model_kwargs,  # model_class-impute.py\ForwardSumModel
            dataset=dataset, epochs=epochs, device=device,
            **kwargs)
        trainer.save_checkpoint(checkpoint_file)  # 保存模型
...
def train_model(
    ...
    trainer = pl.Trainer(
            max_epochs=epochs,
            callbacks=[tracker],
            deterministic=True,  # 设置PyTorch操作是否必须使用确定性算法
