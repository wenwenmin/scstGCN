...
def get_locs(prefix, target_shape=None):
    ...
    if target_shape is not None:
        ...
        rescale_factor = current_shape // target_shape  # 默认是16
        locs = locs.astype(float)
        locs /= rescale_factor  # 就是 spot在特征图上的位置，特征图就是组织学图像缩小16倍----可调

    # find the nearest pixel
    locs = locs.round().astype(int)  # 四舍五入
...
def get_embeddings(prefix):
    ...
    embs = np.concatenate([embs['his'], embs['rgb'], embs['pos']])
    embs = embs.transpose(1, 2, 0)  # C1+C2+3,H,W,
    return embs
