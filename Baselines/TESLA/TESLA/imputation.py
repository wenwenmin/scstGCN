...
def imputation(img, raw, cnt, genes, shape="None", res=50, s=1, k=2, num_nbs=10):
	...
	sudo=sudo[sudo.index.isin([i for i in sudo.index if (binary_enlarged[sudo.y[i], sudo.x[i]]!=0)])]
	b=res
	...
	#Add a threshold
	dis_threshold=np.nanquantile(dis.values, num_nbs/known_adata.shape[0])
	for i in range(sudo_adata.shape[0]):
		...
		if len(nbs)>0:
			...
			#For superpixel have super far nbs, weights=0
			if np.min(nbs.values)>dis_threshold:
				weights=weights*0
